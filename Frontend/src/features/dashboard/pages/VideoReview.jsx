    ...
    try {
      const data = await updateSceneVideo(id, formData, {
        onUploadProgress: (progressEvent) => {
          const total =
            progressEvent.total || progressEvent?.nativeEvent?.total || 1;
    ...
    try {
      ...
      const nextComment = {
        ...
        displayTime: formatTime(
          created.timestamp || Math.floor(currentVideoTime),
        ),
      };

      setComments((prev) =>
        [...prev, nextComment].sort((a, b) => a.time - b.time),
      );
      setNewComment("");
      ...
      <div className="h-16 flex shrink-0 items-center px-6 border-b border-neutral-200 dark:border-neutral-800 bg-white/80 dark:bg-[#111]/80 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {isReplacing ? "Uploading..." : "Upload New Version"}
          </button>
          <input
            ref={uploadInputRef}
            type="file"
